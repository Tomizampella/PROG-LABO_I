...
necesarios a la hora de realizar un alambrado permetral, se le solicita al usuario que ingrese el ancho 
y el largo del terreno.
    ...
    esquinas).
    C. Informar la cantidad de postes de quebracho Fino de 2.2 mts (van cada 12 mts lineales, si en es lugar
    no se encuentra el poste grueso).
...
class App(customtkinter.CTk):
    ...
    def btn_calcular_on_click(self):
        ...
        m_cuadrados = largo * ancho
        perimetro = largo * 2 + ancho * 2
        quebracho_g = perimetro // 250 + 4
        quebracho_f = perimetro // 12 - quebracho_g
        varillas = perimetro // 2 
        alambre = perimetro * 7

        mensaje = f"{m_cuadrados} m2\n"\
                f"{perimetro} ml\n"\
                f"Quebracho grueso = {quebracho_g} unidades\n"\
                f"Quebracho fino = {quebracho_f} unidades\n"\
                f"Varillas = {varillas} unidades\n"\
                f"Alambre (considerando 7 hilos) = {alambre} m"
                

        alert(title="Calculo materiales", message = mensaje)
