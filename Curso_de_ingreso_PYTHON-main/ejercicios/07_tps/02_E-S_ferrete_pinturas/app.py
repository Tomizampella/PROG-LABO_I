...
2.	Para el departamento de Pinturas:
	A.	Al ingresar una temperatura en Fahrenheit debemos mostrar la temperatura en Centígrados
      con un mensaje concatenado (0 °F − 32) × 5/9 = -17,78 °C
...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        ...
        temperatura_c_flt = float(temperatura_c_txt)

        cambio_c_f = (temperatura_c_flt * (9 / 5)) + 32 

        mensaje= f"{temperatura_c_flt} C° son {cambio_c_f} F°"
        alert(title="DE C° A F°", message=mensaje)
    
        
    def btn_convertir_f_c_on_click(self):
        temperatura_f_txt = self.txt_temperatura_f.get()
        temperatura_f_flt = float(temperatura_f_txt)

        cambio_f_c = (temperatura_f_flt - 32) * 5/9 

        mensaje = f"{temperatura_f_flt} F° son {cambio_f_c} C°"
        alert(title="DE F° A C°", message=mensaje)
