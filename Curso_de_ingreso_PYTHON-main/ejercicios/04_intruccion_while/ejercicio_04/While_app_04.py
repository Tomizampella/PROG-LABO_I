...
class App(customtkinter.CTk):
    ...
    def btn_validar_numero_on_click(self):
        numero_ingresado = int(prompt(title = "Ingrese el dígito", prompt = "ingrese un número"))
        acceso = True
        
        while numero_ingresado > 9:
             numero_ingresado = int(prompt(title = "Ingrese el dígito", prompt = "ingrese un número"))

        while acceso == True and numero_ingresado <= 9 and numero_ingresado >= 0 :
                mensaje = "Número correcto"
                acceso = False

        alert(title = "Ingrese el dígito", message = mensaje)
