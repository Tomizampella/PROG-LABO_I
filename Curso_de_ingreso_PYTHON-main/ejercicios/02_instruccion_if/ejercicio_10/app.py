...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        nota_aleatoria_alumno = random.randint (1,10)
        nota_aleatoria_numero = int (nota_aleatoria_alumno)

        if nota_aleatoria_numero >= 6:
            estado ="Promoción directa"
        elif nota_aleatoria_numero >= 4:
            estado = "Aprobado"
        else:
            estado= "Desaprobado"

        
        mensaje = f"La nota es {nota_aleatoria_numero}"
        alert(estado, message=mensaje)
