...
def stark_normalizar_datos(lista_personajes: list) -> bool:
    ...
    for heroe in lista_personajes: 
        for clave, valor in heroe.items(): # El Método items() devuelven como tuplas en una lista de la forma (clave, valor)
            valor_sin_decimal = valor.replace (".", "") #Solo para obtener las claves de valor numerico
            
            if valor_sin_decimal.isdigit() == True:
                if "." in valor:
                    if type(valor) != float:
                        heroe[clave] = float(valor)
                         #guardo casteo en el diccionario lista_personajes
                        #print("El Superheroe {2} la clave {0} es de un float de valor {1}\n".format(clave, valor,(heroe["nombre"])))#verifico casteo
                        if flag_dato_modificado == False:
                            flag_dato_modificado = True
                else:
                    if type(valor) != int:
                        heroe[clave] = int(valor) #guardo casteo en el diccionario lista_personajes
                        
                        #print("El Superheroe {2} la clave {0} es de un entero de valor {1}\n".format(clave, valor,(heroe["nombre"])))#verifico casteo
                        if flag_dato_modificado == False:
                            flag_dato_modificado = True
...
def stark_marvel_app(lista_personajes):
    '''Recibire la lista de héroes y se encargará de la ejecución principal de nuestro programa'''
...
def listar_heroes_por_color_ojos ():
    '''Agrupa en lista a superhéroes por color de ojos'''
        ...
        # Diccionario para almacenar superhéroes agrupados por color de ojos
    heroes_por_color_ojos = {}

    # Itera a través de la lista de personajes
    for heroe in lista_personajes:
        color_ojos = heroe["color_ojos"]
        
        # Si el color de ojos no está en el diccionario, inicializa la lista vacía
        if color_ojos not in heroes_por_color_ojos:
            heroes_por_color_ojos[color_ojos] = []
        
        # Agrega el personaje a la lista correspondiente
        heroes_por_color_ojos[color_ojos].append(heroe["nombre"])

    # Imprime los superhéroes agrupados por color de ojos
